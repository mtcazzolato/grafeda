# Author: Mirela Cazzolato, Christos Faloutsos
# Date: March 2023
...
class TemporalGraph():
    def __init__(self, df_ehr, source=SOURCE, destination=DESTINATION, measure=MEASURE, timestamp=TIMESTAMP):
        """
        extract temporal features from nodes: inter-arrival time (iat)
        and duration wrt the 'measure' attribute
        """
        # self.filename = filename
        self.read_input_data(df_ehr, source, destination, measure, timestamp)
    ...
    def read_input_data(self, df_ehr, source, destination, measure, timestamp):
        """
        read input data, with four required columns:
        source, destination, measure and timestamp
        """
        self.df = df_ehr #pd.read_csv(self.filename, usecols=[source, destination, measure, timestamp])
    ...
    def fill_features(self, node_direction=SOURCE, prefix='out_'):
        ...
        # Group by nodes by ahash and get the timestamp to compute the lifetime
        group = df_aux.groupby(by=[node_direction], axis=0)[TIMESTAMP]
        
        # Compute the lifetime
        df_iat_measure[prefix+LIFETIME]  = (group.last() - group.first()).astype("timedelta64[s]")
        ...
        df_iat_measure[prefix+TOTAL_COUNT] = group.size()
