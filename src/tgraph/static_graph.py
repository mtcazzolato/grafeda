...
class StaticGraph():
    ...
    def __init__(self, df_ehr, source=SOURCE, destination=DESTINATION, measure=MEASURE):
        """
        ...
        self.df = df_ehr #pd.read_csv(filename)
    ...
    def fill_degree(self, G):
        """
        for each node in the node_set, computes the degree
        and adds the column to the df_nodes dataframe
        """
        
        degs = [(node, val) for (node, val) in G.degree()] # type is a list
        degs_df = pd.DataFrame(degs, columns=[SOURCE, DEGREE])
